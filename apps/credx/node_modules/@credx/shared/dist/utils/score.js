...
function calculateUNLOQ1Score(loanData, engagement) {
    ...
    const progressRatio = 1 - loanData.outstandingBalance / loanData.originalAmount;
    const progressScore = progressRatio * 200;
    // 4. Financial Buffer (max 200)
    const bufferRatio = (loanData.extraPayment + 20000 - loanData.livingExpenses) /
    ...
    if (engagement.hasActiveStreak)
        engagementScore += 15;
    engagementScore += Math.min(25, engagement.streakMonths * 5);
    if (engagement.hasRecentRedemption)
        engagementScore += 15;
    if (engagement.profileCompleted)
        engagementScore += 10;
    if (engagement.hasReferral)
        engagementScore += 10;
...
function getScoreTier(score) {
    ...
    if (score >= 600)
        return { tier: "Good", color: "#3b82f6", icon_key: "good" };
    if (score >= 400)
