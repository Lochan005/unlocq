...
import { calculatePrepaymentScenario, calculatePrepaymentScenario1B } from "@credx/shared";
...
export default function LumpSumPage() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      doc.text("Unloqs - Lump Sum Prepayment Report", margin, y);
      y += 10;
      ...
      doc.setFont("helvetica", "normal");
      doc.text("Generated by Unloqs - Smart Loan Prepayment Calculator", margin, pageHeight - 10);

      doc.save("Unloqs-Lump-Sum-Prepayment-Report.pdf");
    } catch (error) {
    ...
    >
            ...
            <ResultsReveal>
              ...
              {scenario === '1A' && reduceTenureResult?.tenureReduced && reduceTenureResult.tenureReduced > 0 && (
                <div className="mb-6 text-center">
                  ...
                  <div className="flex items-center justify-center gap-2 text-[#4A4ABF]">
                    <AnimatedNumber
              ...
              <div className="mt-6">
                <button
                  ...
                  className="w-full rounded-lg border-2 border-[#E6E4F5] bg-white/50 px-4 py-3 text-sm font-semibold text-[#0F0F5C] transition-colors hover:border-[#4A4ABF] hover:bg-white/70"
                >
      ...
      {showBonusToast && (
        <div className="fixed bottom-4 right-4 z-50 rounded-xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-white shadow-lg">
          <UnloqsCoin size={16} color="#fbbf24" className="inline-block mr-1" /> +25 coins earned for setting a reminder!
        </div>
      )}
    </motion.div>
  );
