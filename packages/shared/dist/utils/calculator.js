...
function calculatePrepaymentScenario(originalPrincipal, annualRate, originalTenure, monthsPaid, prepaymentAmount) {
    ...
    const outstandingPrincipal = calculateOutstandingPrincipal(originalPrincipal, annualRate, originalTenure, monthsPaid);
    // Remaining tenure before prepayment
    const remainingTenure = originalTenure - monthsPaid;
    // Calculate new tenure after prepayment
    const newTenureAfterPrepayDecimal = calculateNewTenure(outstandingPrincipal.toNumber(), annualRate, emi.toNumber(), prepaymentAmount);
    const newTenureAfterPrepay = Math.max(0, Math.ceil(newTenureAfterPrepayDecimal.toNumber()));
    // Calculate interest saved
    const interestSaved = calculateInterestSaved(emi.toNumber(), remainingTenure, newTenureAfterPrepay, prepaymentAmount);
...
function calculateScenario3(originalPrincipal, currentRate, originalTenure, monthsPaid, prepaymentAmount, newRate, refinanceCost, newTenure) {
    ...
    // Calculate outstanding principal after monthsPaid
    const outstandingPrincipal = calculateOutstandingPrincipal(originalPrincipal, currentRate, originalTenure, monthsPaid);
    // Remaining tenure
    const remainingTenure = originalTenure - monthsPaid;
    // Already paid amount
    ...
    const optionB_status = newRate >= currentRate ? "No benefit (rate not lower)" : undefined;
    ...
    // Rule 5: Otherwise compare all valid options fairly, pick lowest cost
        ...
        const validOptions = Object.keys(costs);
